...
function s() {
  try {
    ...
    centers.map((center) => {
      const numberOfClassrooms = center[3]; const centerName = center[0]; const representative = center[1];
      ...
      const updated = center[9]; const
        classroomsThatAreDone = [];
      ...
      if (classroomsThatAreDone.length) {
        notifyByEmail({
          emailAddresses: [email, supervisorEmail], centerName, coordinador: representative, classrooms: classroomsThatAreDone,
        });
      }
    });
  } catch (e) {
